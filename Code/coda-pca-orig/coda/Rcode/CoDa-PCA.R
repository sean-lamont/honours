...
gPCA = function(data,
  ...
  objpca   = list()
  objpca$A = temp$A
  colnames(objpca$A) = paste('PC', paste(1:dimension), sep = '')
  
  objpca$V = temp$V
  
  objpca$Y = resp$A %*% resp$V
  colnames(objpca$Y) = paste('Y', paste(1:dim(data)[2]), sep = '')
...
#perspective_kl: KL between pserspective measures
perspective_kl = function(X, P) {
  clrX = clr_transform( X )
  checkX = exp( clrX )
  
  clrP = clr_transform( P )
  checkP = exp( clrP )
  pkl = mean(rowSums( checkX * ( clrX - clrP ) - checkX + checkP, dims = 1 ))
  return(pkl)
}

#symmetric perspective kl
sym_perspective_kl = function(X, P){
  spkl = 0.5 * perspective_kl(X,P) + 0.5 * perspective_kl(P,X)
  return(spkl)
}